...
class ReviewController extends Controller
{
    ...
    // single item
    public function single( Sites $site ) {
        ...
        // store current url in session
         Session::put('reviewURL', url()->full());
        ...
        // GET META TAGS FROM WEBSITE FOR SEO - SOCIALMEDIA POSTING
        $seoReviewMetaTag = SeoGen::seoSitePosting($review);

    	return view( 'review-single', compact( 'reviews', 'review', 'averageRating', 
                                                'seo_title', 'alreadyReviewed', 'seoReviewMetaTag' ) );

    }

    // take review
    public function takeReview( Sites $site, Request $request ) {
        ...
        try{
            ...
            $data[ 'message' ] = sprintf(__('New review to %s
                                  Reviewer: %s
                                  Title: %s
                                  Rating: %s
                                  Review Content: %s'), 
