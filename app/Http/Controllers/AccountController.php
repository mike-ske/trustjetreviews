...
class AccountController extends Controller
{
    ...
    public function setCompanyWidgetColors( Request $r ) {
        ...
        if( !$company )
            return response()->json([ 'error' => __('No company for your account') ]);

        // validate fields
        $allowedField = [ 'generalBG', 'generalFC', 'testiGB', 'testiFC', 'urlFC' ];

        if(!in_array( $r->field, $allowedField ))
            return response()->json([ 'error' => __('Invalid field name') ]);
    ...
    public function myReviews(  ) {
        ...
        // set seo title
        $seo_title = __('My Reviews') . ' - ' . env( 'APP_NAME' );
    ...
    public function deleteReview( $reviewId ) {
        ...
        try {
            ...
            // set message
            alert()->success(__('Successfully removed review'), __('Success'));
    ...
    public function updateProfile( Request $r ) {
        ...
        // does it have a profile pic?
        if( $r->hasFile( 'profilePic' ) ) {

            $this->validate($r, ['profilePic' => 'required|image|mimes:jpeg,png,jpg|max:2048']);

            $image = $r->file('profilePic');
    ...
    public function mybilling(  ) {
       ...
       // if user claims field is nulled, redirect to browse companies
        if( auth()->user()->claims == null ) {
            alert()->info('Select a Company to Claim then you will be redirected to billing portal.');
    ...
    public function cancelSubscription( Subscriptions $s ) {
        ...
        try {
            ...
            if( auth()->user()->id != $s->user_id )
                throw new \Exception( __('You do not seem to own this subscription id') );
