...
class FrontendState
{
    ...
    public function current($type, Model $billable)
    {
        ...
        return [
            ...
            'nextPayment' => self::subscriptionIsActive($subscription) ? optional($subscription->nextPayment())->toArray() : null,
    ...
    protected function getPlans($type, $billable)
    {
        ...
        foreach ($paddlePlans as $plan) {
            if ($sparkPlan = $plans->where('id', $plan->product_id)->first()) {
                ...
                $sparkPlan->price = Cashier::formatAmount((int) $price, $plan->currency);
    ...
    protected static function parseAmount($amount, $currency)
    {
        ...
        $numberFormatter = new NumberFormatter(
            config('cashier.currency_locale', 'en'), NumberFormatter::DECIMAL
        );
