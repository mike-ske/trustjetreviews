...
<form method="POST" enctype="multipart/form-data" action="{{ route('manageCompanyProcess') }}">
  ...
  <dl>
    <dt>{{ __('Company URL') }}</dt>
    <dd><a href="/reviews/{{ $company->url }}" target="_blank">{{ $company->url }}</a></dd>
    
    <dt>{{ __('Company Category') }}</dt>
    <dd>
      <select class="form-control" name="category_id">
      @foreach( $categories as $c )
        <option value="{{ $c->id }}" @if($company->category_id == $c->id) selected @endif>{{ $c->name }}</option>
      @endforeach
      </select>
    </dd>
