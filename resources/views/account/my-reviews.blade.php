...
<thead>
	...
	<tr>
		<td>
			<a href="{{ route('reviewsForSite', $p->site->url ?? 'No url set') }}">
				{{ $p->site->business_name ?? 'No business name' }}<br>
				<small>{{ $p->site->url ?? 'No url set' }}</small>
			</a>
		</td>
		<td>
			{{ $p->review_title }}
...
@else
	<div class="jumbotron">
		<h4>{{ __("You didn't post any reviews yet.") }}</h4>
	</div>
@endif
</div><!-- /.card -->
</div><!-- /.col-10 -->
